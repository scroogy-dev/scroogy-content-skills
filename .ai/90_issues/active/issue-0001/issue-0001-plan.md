...
- [ ] 완료
- **목표**: 설명·논설형 근거·출처 정책과 네이버 블로그 출력 포맷(자리표시자 교차)을 SKILL.md에 기술한다.
- **작업 내용**:
  1. 근거·출처 정책: 사실 주장은 출처로 뒷받침, 미확보 주장은 추측 표시/제외, 하단 `출처` 섹션 + 상단 `목차`
  2. 출력: 본문 + 이미지 자리표시자 교차 배치, 파일명·순서 표기. 서사형/설명·논설형 출력 예시 2종
- **완료 기준**: `grep -q '목차' blog-photo-draft/SKILL.md && grep -q '출처' blog-photo-draft/SKILL.md && grep -q '근거' blog-photo-draft/SKILL.md`, 출력 예시 코드블록 2개 이상 존재
