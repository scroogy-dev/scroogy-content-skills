...
- **작업 내용**:
  ...
  2. 템플릿 해석 순서: `--template <path>`(외부) → 생략 시 추론 유형의 **내장 기본 골격**(`blog-photo-draft/templates/<type>.md`) 폴백. `--template narrative|expository`로 내장 직접 지정 가능 *(설계 변경 4로 내장은 `how-to` 1종으로 대체됨 — 현행 직접 지정은 `--template how-to`. 본 줄은 Task 3 당시 기록)*
